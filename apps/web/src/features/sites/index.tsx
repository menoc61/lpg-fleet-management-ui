...
function SitesTableCore({ rows, role, onAction }: { rows: SiteRow[]; role: SiteRole; onAction: (row: SiteRow, req: TransitionRequest) => void }) {
  const columns = useMemo<ColumnDef<SiteRow>[]>(() => [
    ...
    { id: 'actions', header: '', cell: ({ row }) => <SiteActionsMenu row={row.original} role={role} onAction={(req) => onAction(row.original, req)} />, meta: { label: 'Actions' }, enableSorting: false, enableHiding: false },
  ], [role, onAction])
...
export function SitesScreen({ kind, role }: { kind: 'site' | 'client_site'; role: SiteRole }) {
  const initial = kind === 'site' ? getSiteRows() : getClientSiteRows()
  ...
  return (
    <PageShell>
      <PageHeader title={title} description={`${rows.length} entrée(s). Statuts: UNASSIGNED → ASSIGNED → ACTIVE → VERIFIED. SUSPENDED/REJECTED accessibles par AGENT/ADMIN/SUPERADMIN.`} />
      <SectionCard><SitesTableCore rows={rows} role={role} onAction={handleAction} /></SectionCard>
    </PageShell>
  )
}
